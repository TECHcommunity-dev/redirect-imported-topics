...
/*
*Modified below function by Webdirekt on Date: 17-08-2020
*To Open link New tab for Discourse Header Search Result.
*/
  ...
  return createWidget(`search-result-${type}`, {
    ...
    html(attrs) {
      return attrs.results.map(r => {
		...
		let target;
        if (type === "topic") {
		  //Added by Webdirekt on Date: 17-08-2020
		  //If Link is for Discourse Header Search result then Open link New tab
		  target = "_blank"; 
